...
class Iterator:
    ...
    def __next__(self):
        if self.step > 0:
            if self.start > self.stop:# проверка  начальных условий для выхода из цикла при "-" шаге
                raise StopIteration()
        ...
        if self.flag:
            self.flag=False # проверка для возврата значения start (первое значение выкидывается иначе)
            return self.pointer

        self.pointer+=self.step# увеличиваем итератор

        if self.step>0:
            if self.pointer>self.stop: # проверка на выход из цикла при "+" шаге
                raise StopIteration()
